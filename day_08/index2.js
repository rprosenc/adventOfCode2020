...
bootCode = bootCodeRaw.split('\n')


// move local variable outside to see how programm terminated
running = true;
programm = function(code) {
	let accumulator = 0;
	...
	while (running) {
		...
		[opCode, arg] = code[lineCounter].split(' ')
		arg = parseInt(arg);
...
for(i=0; i<bootCode.length; i++) {
	[opCode, arg] = bootCode[i].split(' ')
	if (opCode==='nop' || opCode==='jmp') {
		/// switch, test
		variantCode = bootCode.map(l=>l);
		if (opCode==='nop') {
