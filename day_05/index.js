...
pid = function(pass) {
	...
	row = [range[0]==range[1]] ? range[0] : console.log('error for row in pass '+pass)
	range = [0, 7]
 	for(i=7; i<=9; i++) {
		//console.log(range, pass[i])
		range = half(range, pass[i])
	}
	col = [range[0]==range[1]] ? range[0] : console.log('error for col in pass '+pass)
	//return [row, col, (row*8)+col]
	return (row*8)+col;
}

seatPids = passes.map(p => pid(p));
